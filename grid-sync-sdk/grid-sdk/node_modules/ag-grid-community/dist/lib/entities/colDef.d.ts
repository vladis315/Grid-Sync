...
declare type Digit = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9;
declare type NextDigit = [1, 2, 3, 4, 5, 6, 7, 'STOP'];
declare type Inc<T> = T extends Digit ? NextDigit[T] : 'STOP';
declare type StringOrNumKeys<TObj> = keyof TObj & (string | number);
declare type NestedPath<TValue, Prefix extends string, TValueNestedChild, TDepth> = TValue extends object ? `${Prefix}.${TDepth extends 'STOP' ? any : NestedFieldPaths<TValue, TValueNestedChild, TDepth>}` : never;
/**
 * Returns a union of all possible paths to nested fields in `TData`.
 */
export declare type ColDefField<TData = any, TValue = any> = TData extends any ? NestedFieldPaths<TData, TValue, 1> : never;
/**
 * Returns a union of all possible paths to nested fields in `TData`.
 */
export declare type NestedFieldPaths<TData = any, TValue = any, TDepth = 0> = {
    [TKey in StringOrNumKeys<TData>]: (TData[TKey] extends TValue ? `${TKey}` : never) | NestedPath<TData[TKey], `${TKey}`, TValue, Inc<TDepth>>;
...
export declare type HeaderLocation = 'chart' | 'columnDrop' | 'columnToolPanel' | 'csv' | 'filterToolPanel' | 'groupFilter' | 'header' | 'model' | 'advancedFilter' | null;
